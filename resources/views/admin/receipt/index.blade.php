                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-role-table-toolbar="base">
                            @if(in_array(78,permissions()))
                                <a href="{{route('admin.receipts.create')}}" type="button" class="btn btn-primary">اضافه فاتوره شراء</a>
                            @endif
                        </div>
                        <div class="d-flex justify-content-end align-items-center d-none" data-kt-role-table-toolbar="selected">
                            ...
                            <button type="button" {{in_array(81, permissions()) ?'' : 'disabled'}}  class="btn btn-danger" data-kt-role-table-select="delete_selected">حذف</button>
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_roles_table">
                            <thead>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="min-w-125px">الاجمالى</th>
                                    <th class="min-w-125px">المسؤول</th>
                                    <th class="min-w-125px">تاريخ الفاتوره</th>
...
@section('js')
<script src="/assets/js/custom/apps/receipts/list/list.js"></script>
    <script>
        setTimeout(function() {
            $('.flash').fadeOut('fast');
        }, 3000);
        $(document).ready(function(event) {
            $.ajax({
                ...
                success: function(response) {
                    if ($.inArray(80, response) !== -1) {
                        $('.edit').show();
                    } else {
                        $('.edit').hide();
                    }
                    if ($.inArray(81, response) !== -1) {
                        $('.delete').show();
                    } else {
                        $('.delete').hide();
                    }
