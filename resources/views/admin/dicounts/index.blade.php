                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-role-table-toolbar="base">
                            <button data-url="{{route('admin.active-discounts')}}" class="btn btn-light-primary me-3 discount_hold">تفعيل الخصومات المؤجله</button>
                            @if(in_array(120,permissions()))
                                <a href="{{route('admin.discounts.create')}}" type="button" class="btn btn-primary">اضافه خصم</a>
                            @endif
                        </div>
                        <div class="d-flex justify-content-end align-items-center d-none" data-kt-role-table-toolbar="selected">
                            ...
                            <button type="button" {{in_array(122, permissions()) ?'' : 'disabled'}}  class="btn btn-danger" data-kt-role-table-select="delete_selected">حذف</button>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_roles_table">
                            <thead>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="min-w-125px">قيمه الخصم</th>
...
@section('js')
<script src="/assets/js/custom/apps/discounts/list/list.js"></script>
    <script>
        setTimeout(function() {
            $('.flash').fadeOut('fast');
        }, 3000);
        $(document).ready(function(event) {
            $.ajax({
                ...
                success: function(response) {
                    if ($.inArray(121, response) !== -1) {
                        $('.edit').show();
                    } else {
                        $('.edit').hide();
                    }
                    if ($.inArray(122, response) !== -1) {
                        $('.delete').show();
                    } else {
                        $('.delete').hide();
                    }
