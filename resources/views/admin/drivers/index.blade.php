...
@section('list')
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-muted">السائقين</li>
    </ul>
@endsection
@section('content')
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                            ...
                            @if(in_array(33,permissions()))
                                <a href="{{route('admin.drivers.create')}}" type="button" class="btn btn-primary">اضافه سائق</a>
                            @endif
                        </div>
...
{{--                            <button type="button" class="btn btn-danger delete-all" data-kt-customer-table-select="delete_selected">حذف</button>--}}
                                ...
                                <button type="button" {{in_array(36, permissions()) ?'' : 'disabled'}}  class="btn btn-danger delete-all" data-kt-customer-table-select="delete_selected">حذف</button>
        ...
        $(document).ready(function(event) {
            $.ajax({
                ...
                success: function(response) {
                    if ($.inArray(35, response) !== -1) {
                        $('.edit').show();
                    } else {
                        $('.edit').hide();
                    }
                    if ($.inArray(36, response) !== -1) {
                        $('.delete').show();
                    } else {
                        $('.delete').hide();
                    }
...
<script>
    function downloadExcel() {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: '{{route('admin.drivers.export')}}',
            success: function (result, status, xhr) {

                var disposition = xhr.getResponseHeader('content-disposition');
                var matches = /"([^"]*)"/.exec(disposition);
                var filename = (matches != null && matches[1] ? matches[1] : 'drivers.xlsx');
