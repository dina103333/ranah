                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-role-table-toolbar="base">
                            @if(in_array(90,permissions()))
                                <a href="{{route('admin.stores.create')}}" type="button" class="btn btn-primary">اضافه مخزن</a>
                            @endif
                        </div>
                        <div class="d-flex justify-content-end align-items-center d-none" data-kt-role-table-toolbar="selected">
                            ...
                            <button type="button" {{in_array(93, permissions()) ?'' : 'disabled'}} class="btn btn-danger" data-kt-role-table-select="delete_selected">حذف</button>
...
@section('js')
<script src="/assets/js/custom/apps/stores/list.js"></script>
    <script>
        setTimeout(function() {
            $('.flash').fadeOut('fast');
        }, 3000);
        $(document).ready(function(event) {
            $.ajax({
                ...
                success: function(response) {
                    if ($.inArray(92, response) !== -1) {
                        $('.edit').show();
                    } else {
                        $('.edit').hide();
                    }
                    if ($.inArray(93, response) !== -1) {
                        $('.delete').show();
                    } else {
                        $('.delete').hide();
                    }
                    if ($.inArray(30, response) !== -1) {
                        $('.show-product').show();
                    } else {
                        $('.show-product').hide();
                    }
