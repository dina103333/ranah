                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-role-table-toolbar="base">
                            @if (in_array(197, permissions()))
                                <a href="{{ route('admin.create-role') }}" type="button" class="btn btn-primary">اضافه
                                    دور</a>
                            @endif
                        </div>
                        <div class="d-flex justify-content-end align-items-center d-none"
                            ...
                            <button {{in_array(199, permissions()) ?'' : 'disabled'}} type="button" class="btn btn-danger"
                                    data-kt-role-table-select="delete_selected">حذف</button>
                        </div>
                    </div>
                </div>
...
@section('js')
    <script src="/assets/js/custom/apps/roles/list/list.js"></script>
    <script>
        setTimeout(function() {
            $('.flash').fadeOut('fast');
        }, 3000);
        $(document).ready(function(event) {
            $.ajax({
                ...
                success: function(response) {
                    if ($.inArray(196, response) !== -1) {
                        $('.edit').show();
                    } else {
                        $('.edit').hide();
                    }
                    if ($.inArray(199, response) !== -1) {
                        $('.delete').show();
                    } else {
                        $('.delete').hide();
                    }
