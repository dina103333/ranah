...
@section('list')
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-muted">الفئات</li>
    </ul>
@endsection
@section('content')
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-role-table-toolbar="base">
                            ...
                            @if(in_array(5,permissions()))
                                <a href="{{route('admin.subcategories.create')}}" type="button" class="btn btn-primary">اضافه فئه</a>
                            @endif
                        </div>
                        <div class="d-flex justify-content-end align-items-center d-none" data-kt-role-table-toolbar="selected">
                            ...
                            <button type="button" {{in_array(8, permissions()) ?'' : 'disabled'}} class="btn btn-danger" data-kt-role-table-select="delete_selected">حذف</button>
                        </div>
                    </div>
                </div>
...
@section('js')
<script src="/assets/js/custom/apps/subcategories/list/list.js"></script>
    <script>
        setTimeout(function() {
            $('.flash').fadeOut('fast');
        }, 3000);
        $(document).ready(function(event) {
            $.ajax({
                ...
                success: function(response) {
                    if ($.inArray(7, response) !== -1) {
                        $('.edit').show();
                    } else {
                        $('.edit').hide();
                    }
                    if ($.inArray(8, response) !== -1) {
                        $('.delete').show();
                    } else {
                        $('.delete').hide();
                    }
