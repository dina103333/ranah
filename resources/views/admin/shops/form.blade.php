...
<div class="modal-body py-10 px-lg-17" >
    <div class="fv-row mb-7">
        <label class="required fs-6 fw-bold mb-2">اسم نوع المحل</label>
        <input type="text" class="form-control form-control-solid" placeholder="" name="name" value="{{isset($shop_type) ?$shop_type->name :old('name')}}" />
    </div>
    @if ($errors->has('name'))
    ...
    <div id="kt_modal_add_customer_billing_info" class="collapse show">
        <div class="d-flex flex-column mb-7 fv-row">
            ...
            <select name="status" aria-label="Select a Country" data-placeholder="اختر الحاله" data-control="select2"  class="form-select form-select-solid fw-bolder">
                ...
                @foreach ($status as $s)
                    <option value="{{$s}}" {{isset($shop_type) ?( $s == $shop_type->status ?'selected' : '') : ''}}>{{$s}}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<div class="modal-footer flex-center">
    <button type="submit"  class="btn btn-primary">
        <span class="indicator-label">{{isset($shop_type) ? 'تعديل  ' : 'حفظ'}}</span>
    </button>
</div>
